...
and generate clawdag-go workflows interactively through an AI assistant rather than running the
driver CLI directly.
...
and follow the installation instructions in `skills/README.md`.

### Building a release bundle

Package the `skills/` directory into a versioned zip from the repo root.
...
1. **Regenerate `library.md`** — run `go run ./tools/genlibdesc/main.go` to update both
   `skills/clawdag-design/references/library.md` and `skills/clawdag-codegen/references/library.md`
   with any new or changed op descriptions.

2. **Update version references** — three files need the new version string:
   - `skills/clawdag-design/SKILL.md` — `version:` and `library_version:` in frontmatter
   - `skills/clawdag-codegen/SKILL.md` — same frontmatter fields, plus the `require github.com/akennis/clawdag-go` line in the go.mod template in the Steps section
   - `skills/README.md` — the "This bundle targets …" line at the top

3. **Update the dagor replace directive** if `github.com/akennis/dagor` has been tagged at a new
   version — update the `replace github.com/wwz16/dagor =>` line in the go.mod template in
   `skills/clawdag-codegen/SKILL.md` to match.

4. **Build and publish the zip** using the commands above, named with the new version.
...
├── tools/
│   └── genlibdesc/
│       └── main.go       — writes skills/*/references/library.md from op description constants
├── skills/               — downloadable Claude Code skill bundle (see release checklist above)
│   ├── README.md         — end-user install instructions
│   ├── clawdag-design/   — DAG design skill
│   └── clawdag-codegen/  — Go code generation skill
├── examples/
